...
Bigint *set_ones(Bigint *b, int n MTd)
{
	int k;
	uint32_t *x, *xe;
...
static int
    rvOK
    (U * d, const FPI * fpi, int32_t * exp, uint32_t * bits, int exact, int rd,
     int *irv MTd) {
	Bigint *b;
...
int strtodg
    (const char *s00, char **se, const FPI * fpi, int32_t * exp,
     uint32_t * bits) {
	int abe, abits, asub;
