...
char *g_ddfmt_p(char *buf, double *dd0, int ndig, size_t bufsize, int nik)
{
	FPI fpi;
	char *b, *s, *se;
...
#define Rounding FPI_Round_near
 ...
 nanret:
			...
			if (nik > 5 && (nik < 12 || L[_1] != NanDflt_d_D2A[0]
					|| (L[_0] ^ NanDflt_d_D2A[1]) & 0xfffff
					|| L[2 + _1] != NanDflt_d_D2A[0]
					|| (L[2 + _0] ^ NanDflt_d_D2A[1]) &
					0xfffff)) {
				bits0[0] = L[2 + _1];
				bits0[1] =
				    (L[2 + _0] & 0xfffff) | (L[_1] << 20);
				bits0[2] = (L[_1] >> 12) | (L[_0] << 20);
				bits0[3] = (L[_0] >> 12) & 0xff;
				b = add_nanbits(b, bufsize - (b - buf), bits0,
						4);
			}
			return b;
		}
