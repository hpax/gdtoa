...
int main(void)
{
 ...
 fmt_test:
		...
		if (se1 - obuf1 != se - obuf || strcmp(obuf, obuf1))
			printf
			    ("Botch: g_ffmt_p gives \"%s\" rather than \"%s\"\n",
			     obuf1, obuf);
		if (!dItry)
			continue;
		printf("strtoIf returns %d,",
		       strtoIf(ibuf, &se, &fI[0].f, &fI[1].f));
		...
		if (fI[0].f == fI[1].f) {
			...
			printf("fI[0] = #%lx = %.8g\nfI[1] = #%lx = %.8g\n",
			       UL fI[0].L, fI[0].f, UL fI[1].L, fI[1].f);
			if (fI[0].f == u.f)
				printf("fI[0] == strtof\n");
			else if (fI[1].f == u.f)
				printf("fI[1] == strtof\n");
		...
		case STRTOG_Infinite:
			for (nik = 0; nik < 6; ++nik) {
				se1 =
				    g_ffmt_p(obuf1, &u.f, ndig, sizeof(obuf1),
					     nik);
				printf("g_ffmt_p(...,%d): \"%s\"\n", nik,
				       obuf1);
			}
