...
int main(void)
{
		...
		if (!*s1) {
			...
			if (r == 1) {
				...
				if (i != j || u[0].d != ddI[0].d
					...
					printf
					    ("***strtopdd and strtordd disagree!!\n:");
			}
 ...
 fmt_test:
		...
		if (se1 - obuf1 != se - obuf || strcmp(obuf, obuf1))
			printf
			    ("Botch: g_ddfmt_p gives \"%s\" rather than \"%s\"\n",
			     obuf1, obuf);
		if (!dItry)
			continue;
		printf("strtoIdd returns %d,",
		       strtoIdd(ibuf, &se, &ddI[0].d, &ddI[2].d));
		...
		if (ddI[0].d == ddI[2].d && ddI[1].d == ddI[3].d) {
			if (ddI[0].d == u[0].d && ddI[1].d == u[1].d)
				...
				printf
				    ("ddI[0] == ddI[1] = #%lx %lx + %lx %lx\n= %.17g + %17.g\n",
				     UL ddI[0].L[_0], UL ddI[0].L[_1],
				     UL ddI[1].L[_0], UL ddI[1].L[_1], ddI[0].d,
				     ddI[1].d);
		} else {
